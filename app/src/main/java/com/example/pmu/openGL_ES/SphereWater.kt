...
class SphereWater(private val latitudeBands: Int = 40, private val longitudeBands: Int = 40, val radius: Float = 1.0f) {
    ...
    companion object {
        private const val VERTEX_SHADER_CODE = """
            ...
            void main() {
                // Генерация волн на основе времени, с угловым смещением
                float waveHeightX = sin(a_Position.x * 10.0 + u_Time) * 0.02; 
                float waveHeightZ = sin(a_Position.z * 10.0 + u_Time) * 0.02; 
                float waveHeight = waveHeightX + waveHeightZ; // Суммируем волну по обеим осям

                vec4 modPosition = a_Position;
                modPosition.y += waveHeight;  // Волны на оси Y

                gl_Position = u_MVPMatrix * modPosition;
                v_TexCoord = a_TexCoord;
                v_Offset = waveHeight; // Для передачи в фрагментный шейдер
            }
        """
        ...
        private const val FRAGMENT_SHADER_CODE = """
            ...
            void main() {
                ...
                float waterEffect = abs(v_Offset) * 0.3;
